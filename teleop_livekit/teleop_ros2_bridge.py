#!/usr/bin/env python3
"""
Teleop Bridge - Business logic for robot teleoperation via RTC
...
class TeleopBridge(RTCROS2BridgeBase):
    ...
    def __init__(self, rtc: RTCInterface, node_name: str = 'teleop_bridge'):
        super().__init__(rtc, node_name)
    ...
    def image_callback(self, msg: Image):
        ...
        try:
            ...
            if len(self._rtc.remote_participants) == 0:
                self.get_logger().info("No participants connected, skipping frame upload", throttle_duration_sec=5)
                return False
            
            # Convert ROS Image to VideoFrame
            frame_bytes = bytearray(msg.data)
            frame = VideoFrame(
                width=msg.width,
                height=msg.height,
                buffer_type=VideoBufferType.RGB24,
                data=frame_bytes
            )
            if self._video_source is not None:
                self._rtc.capture_video_frame(self._video_source, frame)
    ...
    def _create_video_track(self):
        ...
        self._video_source = self._rtc.create_video_source(640, 480)
        ...
        self._submit_to_loop(
            self._rtc.publish_video_track("wrist_camera", self._video_source, encoding)
        )
    ...
    def compressed_pointcloud_callback(self, msg: ByteMultiArray):
        """Process a ROS CompressedPointCloud2 message: send as chunked binary over RTC."""
        try:
    ...
    def odometry_callback(self, msg: Odometry):
        """
        Send odometry pose to RTC with:
